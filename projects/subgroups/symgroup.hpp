...
public:
    ...
    SymGroup(std::vector<SymOpType> generating_elements, const MultiplyType& multiply_elements);
...
SymGroup<SymOpType, BinaryCompareType> operator*(SymGroup<SymOpType, BinaryCompareType> lhs,
                                                 const SymGroup<SymOpType, BinaryCompareType>& rhs);
...
{
    // TODO: Think of a way of not repeating this
    auto compare_lambda = [this, new_operation](const SymOpType& other) { return this->binary_comparator(other, new_operation); };
...
template <typename SymOpType, typename BinaryCompareType>
template <typename... Args>
SymGroup<SymOpType, BinaryCompareType>::SymGroup(std::vector<SymOpType> generating_elements, Args... args)
    : SymGroup<SymOpType, BinaryCompareType>(generating_elements, BinaryCompareType(args...))
{
}

template <typename SymOpType, typename BinaryCompareType>
SymGroup<SymOpType, BinaryCompareType>::SymGroup(std::vector<SymOpType> generating_elements, const BinaryCompareType& binary_comparator)
...
bool SymGroupCompare_f<SymOpType, BinaryCompareType>::operator()(const SymGroup<SymOpType, BinaryCompareType>& group2) const
{
    ...
    else
    {
        ...
        {
            auto compare_elements = [symop, b_compare](const SymOpType& other) { return b_compare(symop, other); };
