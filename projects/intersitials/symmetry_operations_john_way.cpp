...
std::vector<Eigen::Vector3d> get_translations(const std::vector<Site>& original_basis, const std::vector<Site>& transformed_basis)
{

            std::vector<Site> transformed_translated_basis;
            Eigen::Vector3d trans;
	    std::vector<Eigen::Vector3d> all_potential_translations;
	    //refactor
		    ...
		    //may want find if on potential translations to make sure they are unique
		    all_potential_translations.push_back(trans);
                }
...
std::vector<SymOp> expand_with_translations(SymOp single_point_group_op, std::vector<Eigen::Vector3d> all_translations)
{
	//for every translation make a new symop that's point group op with trans
	std::vector<SymOp> all_potential_symops;
	for (auto& trans: all_translations)
	{
		all_potential_symops.emplace_back(single_point_group_op.get_cart_matrix(), trans);
	}
	return all_potential_symops;
}
...
std::vector<SymOp> find_factor_group(Structure my_struc)
{
    ...
    for (SymOp point_group_op : point_group)
    {
 	   ...
 	   //auto transformed_translated_basis_and_translation= get_transformed_translated_basis_and_translation(Basis, transformed_basis, my_struc.get_lattice());  
      	   auto all_translations= get_translations(Basis, transformed_basis);
	   auto get_expanded_ops= expand_with_translations(point_group_op, all_translations);
           std::vector<Site> transformed_translated_basis;
	   for (int k=0; k<get_expanded_ops.size(); k++)
	   {
		   for (int l=0; l<transformed_basis.size(); l++)
		  {
		   	Eigen::Vector3d changed_basis=transformed_basis[l].get_eigen_coordinate()+get_expanded_ops[k].get_translation();
		   	transformed_translated_basis.push_back(Site(transformed_basis[l].get_atom(), changed_basis));
		  }

    	          if (basis_maps_onto_itself(Basis, transformed_translated_basis, my_struc.get_lattice()))
   	  	    {
        		factor_group.emplace_back(point_group_op.get_cart_matrix(), get_expanded_ops[k].get_translation());
